function  [P, C, W, iter, T] =prgls_GRBF_sparse(X, Y, beta, lambda, max_it, tol, viz, outliers, corresp,sigma2, t, nsc)

[N, D]=size(X); [M, D]=size(Y);

K = 25;
% Initialization
T=Y; iter=0;  ntol=tol+10; W=zeros(K,D);
...
idx = idx(1:min(K,size(tmp_Y,1)));ctrl_pts=tmp_Y(idx,:);
% Construct affinity matrix G
G = cpd_G(ctrl_pts,ctrl_pts,beta);
U = cpd_G(Y, ctrl_pts, beta);
...
%     disp('done.')
    ...
    idx = sum(P_prior,2) < 1;
    P_prior(idx,:) = 1/M;
    if viz, figure(1); cpd_plot_iter(X, T); axis off; end;
    end
    ...
    W = (U'*dP*U + lambda*sigma2*G)\(U'*PX-U'*dP*Y);
