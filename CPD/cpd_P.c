...

{
  ...
  ksig = -2.0 * *sigma2;
  outlier_tmp=(*outlier*pow (-ksig*3.14159265358979,0.5*D))/((1-*outlier)*N); 
 /* printf ("ksig = %lf\n", *sigma2);*/
  ...
  for (n=0; n < N; n++) {
      ...
      for (m=0; m < M; m++) {
          ...
          *(P+m)=exp(razn/ksig)* *(s_matrix+n+m*N);
          sp+=*(P+m);
      }
...
{
  double *x, *y, *sigma2, *outlier, *s_matrix, *P1, *Pt1, *Px, *E;
  int     N, M, D;
  ...
  cpd_comp(x, y, sigma2, outlier, P1, Pt1, Px, E, s_matrix, N, M, D);
